...
const caseStudyData = JSON.parse(
  fs.readFileSync(path.join(__dirname, '../case-study-savannah.json'), 'utf8')
);
...
async function seedSavannahCaseStudy() {
  console.log('Seeding Savannah case study...');
  ...
  if (existing) {
    ...
    console.log('✅ Successfully updated Savannah case study!');
    ...
    console.log('✅ Successfully seeded Savannah case study!');
    console.log(`   ID: ${data.id}`);
    console.log(`   Slug: ${data.slug}`);
    console.log(`   View at: /case-studies/${data.slug}`);
  }
